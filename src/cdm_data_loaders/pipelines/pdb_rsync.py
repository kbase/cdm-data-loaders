...
def download_and_stage(
    *,
    ...
    with tempfile.TemporaryDirectory() as _tmpdir:
        ...
        with tqdm.tqdm(
            total=len(pdb_ids), unit="entry", desc="Downloading & staging", smoothing=0.01
        ) as pbar:
            with ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        if not dry_run:
            ...
            with report_path.open("w") as f:
                json.dump(report, f, indent=2)
            upload_file(report_path, f"{staging_bucket}/{staging_key_prefix.rstrip('/')}", show_progress=False)
            staged_objects += 1
