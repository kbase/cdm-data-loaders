"""Unit tests for cdm_data_loaders.sifts.metadata."""
...
import cdm_data_loaders.sifts.metadata as metadata_mod
...
_FILENAMES = ["pdb_chain_uniprot.tsv.gz", "pdb_chain_go.tsv.gz"]
...
def test_create_descriptor() -> None:
    ...
    d = create_descriptor(_FILENAMES, _SAMPLE_RESOURCES, timestamp=_TIMESTAMP)

    assert d["identifier"] == "EBI_SIFTS:mappings"
    assert d["resource_type"] == "dataset"
    assert d["url"] == "https://www.ebi.ac.uk/pdbe/docs/sifts/"
    assert d["license"] == {}
    assert d["contributors"][0]["name"] == "European Bioinformatics Institute"
    assert d["publisher"]["organization_id"] == "ROR:02catss52"

    # Version is derived from timestamp as ISO date
    assert d["version"] == "2023-11-14"

    # Description mentions filenames
    desc = d["descriptions"][0]["description_text"]
    for fname in _FILENAMES:
        assert fname in desc

    # Meta fields
    assert d["meta"]["saved_by"] == "cdm-data-loaders-sifts"
    assert d["meta"]["credit_metadata_schema_version"] == "1.0"
    assert d["meta"]["timestamp"] == _TIMESTAMP
    src = d["meta"]["credit_metadata_source"][0]
    assert src["source_name"] == "EBI SIFTS FTP"
    assert src["access_timestamp"] == _TIMESTAMP

    # Resources
    r0 = d["resources"][0]
    assert r0["hash"] == "md5:abc123"
    assert r0["bytes"] == 4096
    assert r0["name"] == "pdb_chain_uniprot.tsv.gz"

    r1 = d["resources"][1]
    assert "hash" not in r1
    assert r1["bytes"] == 2048
...
def test_create_descriptor_default_timestamp_is_recent() -> None:
    ...
    d = create_descriptor(_FILENAMES, _SAMPLE_RESOURCES)
    after = int(time.time())
...
def test_create_descriptor_resource_name_lowercased() -> None:
    ...
    resources: list[DescriptorResource] = [
        {"name": "PDB_CHAIN_UNIPROT.TSV.GZ", "path": "s3://b/a", "format": "gz", "bytes": 100, "hash": "md5:x"},
    ]
    d = create_descriptor(_FILENAMES, resources, timestamp=_TIMESTAMP)
    assert d["resources"][0]["name"] == "pdb_chain_uniprot.tsv.gz"


def test_create_descriptor_null_bytes_omitted() -> None:
    """Resources with bytes=None have the 'bytes' key removed."""
    resources: list[DescriptorResource] = [
        {"name": "f.tsv.gz", "path": "s3://b/f", "format": "gz", "bytes": None, "hash": "md5:x"},
    ]
    d = create_descriptor(_FILENAMES, resources, timestamp=_TIMESTAMP)
...
def test_create_descriptor_empty_resources() -> None:
    ...
    d = create_descriptor(_FILENAMES, [], timestamp=_TIMESTAMP)
    assert d["resources"] == []
...
def test_validate_descriptor_valid() -> None:
    ...
    validate_descriptor(create_descriptor(_FILENAMES, _SAMPLE_RESOURCES, timestamp=_TIMESTAMP))
...
@pytest.fixture
def mock_s3(monkeypatch: pytest.MonkeyPatch) -> Generator[botocore.client.BaseClient, None, None]:
...
def mock_s3_with_descriptor(mock_s3: botocore.client.BaseClient):
    ...
    descriptor = create_descriptor(_FILENAMES, _SAMPLE_RESOURCES, timestamp=_TIMESTAMP)
