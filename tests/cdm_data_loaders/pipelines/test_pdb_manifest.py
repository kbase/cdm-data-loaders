...
def test_generate_manifest_data_new_field(
...
def test_generate_manifest_data_updated_field(
    current: dict[str, PDBRecord],
    previous: dict[str, PDBRecord],
...
def test_generate_manifest_data_removed_field(
    current: dict[str, PDBRecord],
    removed_set: set[str],
    previous: dict[str, PDBRecord],
...
def test_generate_manifest_data_missing_dates_propagated() -> None:
...
def test_generate_manifest_data_new_and_updated_are_disjoint() -> None:
...
def test_extract_id_from_s3_key(key: str, expected: str | None) -> None:
...
def test_holdings_snapshot_round_trip(tmp_path: Path, records: dict[str, PDBRecord]) -> None:
...
def test_holdings_snapshot_missing_last_modified_defaults_to_empty_string(tmp_path: Path) -> None:
...
def _build_mock_s3(pages: list[list[str]]) -> MagicMock:
...
def test_generate_snapshot_from_s3_state_empty_store_returns_empty_dict() -> None:
    ...
    with patch.object(client, "get_s3_client", return_value=mock_s3):
        result = _generate_snapshot_from_s3_state(PurePosixPath("bucket"), PurePosixPath("prefix"), date(2024, 1, 1))
    assert result == {}


def test_generate_snapshot_from_s3_state_returns_record_for_each_valid_key() -> None:
    ...
    mock_s3 = _build_mock_s3([keys])
    with patch.object(client, "get_s3_client", return_value=mock_s3):
        result = _generate_snapshot_from_s3_state(PurePosixPath("bucket"), PurePosixPath("prefix"), date(2024, 1, 1))
    assert set(result.keys()) == {"pdb_00001abc", "pdb_00001def"}


def test_generate_snapshot_from_s3_state_sets_provided_date_on_all_records() -> None:
...
def test_generate_snapshot_from_s3_state_deduplicates_multiple_objects_per_pdb_id() -> None:
...
def test_generate_snapshot_from_s3_state_skips_keys_without_valid_pdb_id() -> None:
...
def test_generate_snapshot_from_s3_state_aggregates_records_across_multiple_pages() -> None:
...
def test_generate_snapshot_from_s3_state_paginator_called_with_correct_bucket_and_prefix() -> None:
...
def test_generate_snapshot_from_s3_state_page_with_no_contents_key_is_handled_gracefully() -> None:
...
_SNAPSHOT_RECORDS: Final[dict[str, PDBRecord]] = {
    _ID_A: PDBRecord(id=_ID_A, last_modified=_DATE_NEW),
    _ID_B: PDBRecord(id=_ID_B, last_modified=_DATE_OLD),
}


def _writing_side_effect(records: dict[str, PDBRecord]) -> Callable[..., None]:
...
def test_download_holdings_snapshot_returns_parsed_snapshot_on_success() -> None:
    """A successful download returns the correctly deserialised snapshot dict."""
    mock_s3 = MagicMock()
    mock_s3.download_file.side_effect = _writing_side_effect(_SNAPSHOT_RECORDS)
    with patch.object(client, "get_s3_client", return_value=mock_s3):
        result = _download_holdings_snapshot(PurePosixPath("bucket"), PurePosixPath("path/snapshot.json.gz"))
    assert result == _SNAPSHOT_RECORDS


def test_download_holdings_snapshot_calls_download_file_with_correct_bucket_and_key() -> None:
    """download_file is invoked with the exact bucket and key passed to the function."""
    mock_s3 = MagicMock()
    mock_s3.download_file.side_effect = _writing_side_effect({})
    with patch.object(client, "get_s3_client", return_value=mock_s3):
...
def test_download_holdings_snapshot_client_error_raises_value_error() -> None:
...
def test_download_holdings_snapshot_error_message_includes_bucket_and_key() -> None:
...
def test_download_holdings_snapshot_temp_file_is_cleaned_up_on_success() -> None:
...
def test_download_holdings_snapshot_temp_file_is_cleaned_up_on_error() -> None:
...
def test_download_holdings_files_downloads_and_parses_all_three_files() -> None:
...
def test_save_manifest_files_file_contents(
    tmp_path: Path, data: ManifestData, filename: str, expected_lines: list[str]
) -> None:
...
_RUN_REMOVED: Final[dict[str, PDBRecord]] = {}


@pytest.fixture(autouse=True)
def _patch_holdings_download(monkeypatch: pytest.MonkeyPatch) -> None:
    """Replace HTTP downloads with controlled in-memory data."""
    mock_raw = {
        HoldingsFileTypes.CURRENT: dict(_RUN_CURRENT),
        HoldingsFileTypes.LAST_MODIFIED: dict(_RUN_LAST_MODIFIED),
        HoldingsFileTypes.REMOVED: dict(_RUN_REMOVED),
    }
    monkeypatch.setattr(pdb_manifest_mod, "_download_holdings_files", lambda: mock_raw)


def _make_config(tmp_path: Path, **kwargs: object) -> PdbManfestSettings:
    defaults: dict = {
        "bootstrap_date": None,
        "skip_diff": False,
        "regex_filter": None,
...
def test_run_manifest_generation_skip_diff_all_records_are_new(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    run_manifest_generation(_make_config(tmp_path, skip_diff=True))

    mock_dl.assert_not_called()
    transfer = _read_manifest(tmp_path, "transfer_manifest.txt")
    assert sorted(transfer) == sorted(_RUN_CURRENT.keys())
    assert _read_manifest(tmp_path, "updated_manifest.txt") == []
    assert _read_manifest(tmp_path, "removed_manifest.txt") == []


def test_run_manifest_generation_bootstrap_date_calls_generate_snapshot(
    ...
    run_manifest_generation(_make_config(tmp_path, bootstrap_date=bootstrap_date))
...
def test_run_manifest_generation_normal_mode_downloads_snapshot(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    ...
    run_manifest_generation(_make_config(tmp_path))
...
def test_run_manifest_generation_regex_filter_limits_transfer_manifest(
    ...
    run_manifest_generation(_make_config(tmp_path, skip_diff=True, regex_filter=regex_filter))

    transfer = _read_manifest(tmp_path, "transfer_manifest.txt")
    assert sorted(transfer) == sorted(expected_ids)


def test_run_manifest_generation_missing_dates_computed_as_current_minus_last_modified(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    ...
    missing = _read_manifest(tmp_path, "missing_dates.txt")
    assert missing == ["pdb_aaaaaaaa"]


def test_run_manifest_generation_summary_file_written(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert total == len(_RUN_CURRENT)
    assert summary["missing_dates"] == 1


def test_run_manifest_generation_output_directory_created_when_missing(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    ...
    run_manifest_generation(_make_config(tmp_path, skip_diff=True, output_path=output_path))
