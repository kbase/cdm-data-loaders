...
def test_sets_so_keepalive() -> None:
...
def test_sets_tcp_keepidle() -> None:
...
def test_connect_and_login(mock_ftp_cls: MagicMock) -> None:
...
def test_sends_noop_when_idle() -> None:
...
def test_no_noop_when_recent() -> None:
...
def test_returns_file_list() -> None:
...
def test_retries_on_error_temp() -> None:
...
def test_downloads_file(tmp_path: Path) -> None:
...
def test_download_retries_on_error_temp(tmp_path: Path) -> None:
...
def test_retrieve_text_returns_content() -> None:
...
def test_get_returns_same_connection(mock_connect: MagicMock) -> None:
...
@patch("cdm_data_loaders.utils.file_transfer.ftp_client.connect_ftp")
def test_close_all(mock_connect: MagicMock) -> None:
