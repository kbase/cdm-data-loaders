...
from cdm_data_loaders.utils.file_transfer.progress import (
...
def test_make_progress_bar_pass_total_int_is_stored() -> None:
    ...
    bar = make_progress_bar(total=12345, desc="test")
    try:
        assert bar.total == 12345
    finally:
        bar.close()
...
def test_make_progress_bar_fail_missing_required_arguments() -> None:
    """total and desc are required keyword arguments; omitting them raises TypeError."""
    with pytest.raises(TypeError):
        make_progress_bar()  # type: ignore[call-arg]

    with pytest.raises(TypeError):
        make_progress_bar(total=10)  # type: ignore[call-arg]

    with pytest.raises(TypeError):
        make_progress_bar(desc="test")  # type: ignore[call-arg]
