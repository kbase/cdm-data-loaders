...
def test_compute_file_checksum_pass_hashlib_algorithm_matches_hashlib(
    ...
    assert compute_file_checksum(file_path, algorithm) == expected


@pytest.mark.parametrize("algorithm", ["MD5", "Sha256", "SHA512"])
def test_compute_file_checksum_pass_case_insensitive_algorithm(tmp_path: Path, algorithm: str) -> None:
    """Algorithm names are matched case-insensitively, same as validate_checksum_fn."""
    file_path = tmp_path / "data.bin"
    file_path.write_bytes(b"content")
    expected = hashlib.new(algorithm.lower(), b"content").hexdigest()
    assert compute_file_checksum(file_path, algorithm) == expected
...
@pytest.mark.parametrize(
    "content",
    [b"", b"hello", b"x" * (2 * (1 << 20) + 123)],
    ids=["empty", "small", "multi-chunk"],
)
def test_compute_file_checksum_pass_crc64nvme_matches_direct_computation(tmp_path: Path, content: bytes) -> None:
...
def test_compute_file_checksum_pass_crc64nvme_case_insensitive(tmp_path: Path) -> None:
    """crc64nvme is recognised regardless of case, consistently with hashlib algorithm handling."""
    file_path = tmp_path / "data.bin"
    file_path.write_bytes(b"content")
    expected_crc = _crc64nvme(b"content", 0)
    expected = base64.b64encode(expected_crc.to_bytes(8, byteorder="big")).decode()
    assert compute_file_checksum(file_path, "CRC64NVME") == expected


def test_compute_file_checksum_fail_unsupported_algorithm(tmp_path: Path) -> None:
    """An unsupported algorithm name raises ValueError before any file I/O happens."""
    file_path = tmp_path / "data.bin"
    file_path.write_bytes(b"content")
    with pytest.raises(ValueError, match="not supported"):
        compute_file_checksum(file_path, "not_a_real_algorithm")


def test_compute_file_checksum_fail_shake_algorithm(tmp_path: Path) -> None:
    """Shake algorithms are rejected, consistently with validate_checksum_fn."""
    file_path = tmp_path / "data.bin"
    file_path.write_bytes(b"content")
    with pytest.raises(ValueError, match="not supported"):
        compute_file_checksum(file_path, "shake_128")
...
@pytest.mark.parametrize("checksum_fn", ["md5", "sha1", "SHA256", "sha512", "MD5", "Sha256"])
def test_validate_checksum_fn_pass_supported_algorithm(checksum_fn: str) -> None:
    """Supported hashlib algorithms are accepted and returned lowercased."""
    assert validate_checksum_fn(checksum_fn) == checksum_fn.lower()


@pytest.mark.parametrize("checksum_fn", ["not_a_real_algorithm", "banana", ""])
def test_validate_checksum_fn_fail_unsupported_algorithm(checksum_fn: str) -> None:
    """Unsupported algorithm names raise ValueError."""
    with pytest.raises(ValueError, match="not supported"):
        validate_checksum_fn(checksum_fn)


@pytest.mark.parametrize("checksum_fn", ["shake_128", "shake_256", "SHAKE_128"])
def test_validate_checksum_fn_fail_shake_algorithm(checksum_fn: str) -> None:
    """Shake algorithms are explicitly rejected even though hashlib supports them."""
    with pytest.raises(ValueError, match="not supported"):
        validate_checksum_fn(checksum_fn)


@pytest.mark.parametrize("name", ["crc64nvme", "CRC64NVME", "Crc64Nvme"])
def test_validate_checksum_fn_pass_crc64nvme_case_insensitive(name: str) -> None:
    """crc64nvme is recognised regardless of case, same as hashlib algorithm names."""
    assert validate_checksum_fn(name) == "crc64nvme"


"""Tests for resolve_checksum_fn"""


def test_resolve_checksum_fn_pass_no_expected_checksum() -> None:
    """No expected checksum means no algorithm needs resolving, regardless of other args."""
    assert resolve_checksum_fn(None, "sha256", "md5") is None
    assert resolve_checksum_fn("", "sha256", "md5") is None


@pytest.mark.parametrize(
    ("checksum_fn", "default_checksum_fn", "expected"),
    [
        ("sha256", "md5", "sha256"),
        (None, "md5", "md5"),
        (None, "sha1", "sha1"),
...
def test_resolve_checksum_fn_pass_resolves_expected_algorithm(
    checksum_fn: str | None, default_checksum_fn: str, expected: str
) -> None:
    """Explicit checksum_fn takes precedence, falling back to default_checksum_fn when absent."""
    assert resolve_checksum_fn("flash_gordon", checksum_fn, default_checksum_fn) == expected


def test_resolve_checksum_fn_fail_unsupported_algorithm() -> None:
    """An unsupported algorithm still raises ValueError when a checksum needs verifying."""
    with pytest.raises(ValueError, match="not supported"):
        resolve_checksum_fn("flash_gordon", "not_a_real_algorithm", "md5")


def test_resolve_checksum_fn_pass_empty_string_checksum_fn_falls_back_to_default() -> None:
    """An empty-string checksum_fn is treated the same as None and falls back to the default."""
    assert resolve_checksum_fn("flash_gordon", "", "md5") == "md5"


def test_resolve_checksum_fn_fail_unsupported_default_algorithm() -> None:
    """An unsupported default_checksum_fn also raises ValueError when checksum_fn is not supplied."""
    with pytest.raises(ValueError, match="not supported"):
        resolve_checksum_fn("flash_gordon", None, "not_a_real_algorithm")
...
def test_hashingreader_pass_crc64nvme_b64digest_matches_s3_format() -> None:
    ...
    reader.read(-1)
    expected_crc = _crc64nvme(data, 0)
    expected = base64.b64encode(expected_crc.to_bytes(8, byteorder="big")).decode()
    assert reader.b64digest() == expected


def test_hashingreader_pass_crc64nvme_hexdigest_matches_raw_hex_not_base64() -> None:
    """HashingReader.hexdigest() for crc64nvme returns hex, NOT S3's base64 form.

    Documents current behavior so a future fix (e.g. exposing b64digest())
    ...
    assert reader.hexdigest() == expected_crc.to_bytes(8, "big").hex()
